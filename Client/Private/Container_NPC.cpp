#include "stdafx.h"
#include "..\Public\COntainer_NPC.h"

#include "Body_Human.h"
#include "ClientInstance.h"
#include "Tool.h"

CContainer_NPC::CContainer_NPC(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CLongvinter_Container{ pDevice, pContext }
{
}

CContainer_NPC::CContainer_NPC(const CContainer_NPC& Prototype)
	: CLongvinter_Container{ Prototype }
{
}

HRESULT CContainer_NPC::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CContainer_NPC::Initialize(void* pArg)
{
...
void CContainer_NPC::Priority_Update(_float fTimeDelta)
{
...
void CContainer_NPC::Update(_float fTimeDelta)
{
...
void CContainer_NPC::Late_Update(_float fTimeDelta)
{
...
void CContainer_NPC::Collision_Reaction_InterAction(CGameObject* pPoint, INTERACTION eIndex, CInterAction::INTER_INFO& tOpponent)
{
	__super::Collision_Reaction_InterAction(pPoint, eIndex, tOpponent);

}

void CContainer_NPC::Collision_Reaction_MadeInterAction(CGameObject* pPoint, INTERACTION eIndex)
{
	__super::Collision_Reaction_MadeInterAction(pPoint, eIndex);
}

void CContainer_NPC::Collision_Reaction_Container(CGameObject* pPoint, CONTAINER eIndex)
{
	__super::Collision_Reaction_Container(pPoint, eIndex);
}

void CContainer_NPC::DeadAction()
{
	__super::DeadAction();
}


void CContainer_NPC::Moving_Control(_float fTimeDelta)
{
	__super::Moving_Control(fTimeDelta);
}

void CContainer_NPC::Weapon_Control(_float fTimeDelta)
{
...
void CContainer_NPC::Camera_Control(_float fTimeDelta)
{
	__super::Camera_Control(fTimeDelta);
}

void CContainer_NPC::Test_Control(_float fTimeDelta)
{
	__super::Test_Control(fTimeDelta);
}

void CContainer_NPC::Set_AI_Status(_float fTimeDelta)
{
...
HRESULT CContainer_NPC::Ready_Components()
{
...
HRESULT CContainer_NPC::Ready_PartObjects()
{
...
CContainer_NPC* CContainer_NPC::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CContainer_NPC* pInstance = new CContainer_NPC(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CContainer_NPC"));
...
CGameObject* CContainer_NPC::Clone(void* pArg)
{
	CContainer_NPC* pInstance = new CContainer_NPC(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CContainer_NPC"));
		Safe_Release(pInstance);
	}
