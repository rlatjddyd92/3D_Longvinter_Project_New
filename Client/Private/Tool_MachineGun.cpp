#include "stdafx.h"
#include "..\Public\Tool_MachineGun.h"

#include "Player.h"

#include "GameInstance.h"

CTool_MachineGun::CTool_MachineGun(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CTool{ pDevice, pContext }
{
}

CTool_MachineGun::CTool_MachineGun(const CTool_MachineGun& Prototype)
	: CTool{ Prototype }
{
}

HRESULT CTool_MachineGun::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTool_MachineGun::Initialize(void* pArg)
{
...
void CTool_MachineGun::Priority_Update(_float fTimeDelta)
{

	int a = 10;
}

void CTool_MachineGun::Update(_float fTimeDelta)
{
...
void CTool_MachineGun::Late_Update(_float fTimeDelta)
{
...
HRESULT CTool_MachineGun::Render()
{
...
HRESULT CTool_MachineGun::Ready_Components()
{
	/* FOR.Com_Shader */
	...
	/* FOR.Com_Model */
	if (FAILED(__super::Add_Component(_int(LEVELID::LEVEL_STATIC), TEXT("Prototype_Component_Model_MachineGun"),
...
CTool_MachineGun* CTool_MachineGun::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CTool_MachineGun* pInstance = new CTool_MachineGun(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CTool_MachineGun"));
...
CGameObject* CTool_MachineGun::Clone(void* pArg)
{
	CTool_MachineGun* pInstance = new CTool_MachineGun(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CTool_MachineGun"));
		Safe_Release(pInstance);
	}
