#include "stdafx.h"
#include "..\Public\Tool_ChainSaw.h"

#include "Player.h"

#include "GameInstance.h"

CTool_ChainSaw::CTool_ChainSaw(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CTool{ pDevice, pContext }
{
}

CTool_ChainSaw::CTool_ChainSaw(const CTool_ChainSaw& Prototype)
	: CTool{ Prototype }
{
}

HRESULT CTool_ChainSaw::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTool_ChainSaw::Initialize(void* pArg)
{
...
void CTool_ChainSaw::Priority_Update(_float fTimeDelta)
{

	int a = 10;
}

void CTool_ChainSaw::Update(_float fTimeDelta)
{
...
void CTool_ChainSaw::Late_Update(_float fTimeDelta)
{
...
HRESULT CTool_ChainSaw::Ready_Components()
{
	/* FOR.Com_Shader */
	...
	/* FOR.Com_Model */
	if (FAILED(__super::Add_Component(_int(LEVELID::LEVEL_STATIC), TEXT("Prototype_Component_Model_ChainSaw"),
...
CTool_ChainSaw* CTool_ChainSaw::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CTool_ChainSaw* pInstance = new CTool_ChainSaw(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CTool_ChainSaw"));
...
CGameObject* CTool_ChainSaw::Clone(void* pArg)
{
	CTool_ChainSaw* pInstance = new CTool_ChainSaw(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CTool_ChainSaw"));
		Safe_Release(pInstance);
	}
