...
CParticle_Debris::CParticle_Debris(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Particle{ pDevice, pContext }
{
}

CParticle_Debris::CParticle_Debris(const CParticle_Debris& Prototype)
	: CEffect_Particle{ Prototype }
{
}

HRESULT CParticle_Debris::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CParticle_Debris::Initialize(void* pArg)
{
	...
	m_tSpec.fTime = 0.2f;
	m_tSpec.fFly = 0.f;
	m_tSpec.fScale = 0.05f;
	m_tSpec.fDirec = { 0.f,0.2f,0.f };
	m_tSpec.fColor = { 255.f/255.f,140.f/255.f,0.f/255.f,1.f };
...
void CParticle_Debris::Priority_Update(_float fTimeDelta)
{
...
void CParticle_Debris::Update(_float fTimeDelta)
{
...
void CParticle_Debris::Late_Update(_float fTimeDelta)
{
...
HRESULT CParticle_Debris::Render()
{
	//__super::Render();
...
HRESULT CParticle_Debris::Bind_WorldMatrix(CShader* pShader, const _char* pContantName)
{
	return pShader->Bind_Matrix(pContantName, &m_WorldMatrix);
}

HRESULT CParticle_Debris::Ready_Components()
{
	/* FOR.Com_Texture */
...
HRESULT CParticle_Debris::Ready_PartObjects()
{
	return S_OK;
}

CParticle_Debris* CParticle_Debris::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CParticle_Debris* pInstance = new CParticle_Debris(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CParticle_Debris"));
...
CGameObject* CParticle_Debris::Clone(void* pArg)
{
	CParticle_Debris* pInstance = new CParticle_Debris(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CParticle_Debris"));
		Safe_Release(pInstance);
	}
