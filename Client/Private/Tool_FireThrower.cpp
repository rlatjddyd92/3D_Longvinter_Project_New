#include "stdafx.h"
#include "..\Public\Tool_FireThrower.h"

#include "Player.h"

#include "GameInstance.h"

CTool_FireThrower::CTool_FireThrower(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CTool{ pDevice, pContext }
{
}

CTool_FireThrower::CTool_FireThrower(const CTool_FireThrower& Prototype)
	: CTool{ Prototype }
{
}

HRESULT CTool_FireThrower::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTool_FireThrower::Initialize(void* pArg)
{
...
void CTool_FireThrower::Priority_Update(_float fTimeDelta)
{

	int a = 10;
}

void CTool_FireThrower::Update(_float fTimeDelta)
{
...
void CTool_FireThrower::Late_Update(_float fTimeDelta)
{
...
HRESULT CTool_FireThrower::Render()
{
...
HRESULT CTool_FireThrower::Ready_Components()
{
	/* FOR.Com_Shader */
	...
	/* FOR.Com_Model */
	if (FAILED(__super::Add_Component(_int(LEVELID::LEVEL_STATIC), TEXT("Prototype_Component_Model_FireThrower"),
...
CTool_FireThrower* CTool_FireThrower::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CTool_FireThrower* pInstance = new CTool_FireThrower(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CTool_FireThrower"));
...
CGameObject* CTool_FireThrower::Clone(void* pArg)
{
	CTool_FireThrower* pInstance = new CTool_FireThrower(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CTool_FireThrower"));
		Safe_Release(pInstance);
	}
