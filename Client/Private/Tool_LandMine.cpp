#include "stdafx.h"
#include "..\Public\Tool_LandMine.h"

#include "Player.h"

#include "GameInstance.h"

CTool_LandMine::CTool_LandMine(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CTool{ pDevice, pContext }
{
}

CTool_LandMine::CTool_LandMine(const CTool_LandMine& Prototype)
	: CTool{ Prototype }
{
}

HRESULT CTool_LandMine::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTool_LandMine::Initialize(void* pArg)
{
...
void CTool_LandMine::Priority_Update(_float fTimeDelta)
{

	int a = 10;
}

void CTool_LandMine::Update(_float fTimeDelta)
{
...
void CTool_LandMine::Late_Update(_float fTimeDelta)
{
...
HRESULT CTool_LandMine::Ready_Components()
{
	/* FOR.Com_Shader */
...
CTool_LandMine* CTool_LandMine::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CTool_LandMine* pInstance = new CTool_LandMine(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CTool_LandMine"));
...
CGameObject* CTool_LandMine::Clone(void* pArg)
{
	CTool_LandMine* pInstance = new CTool_LandMine(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CTool_LandMine"));
		Safe_Release(pInstance);
	}
