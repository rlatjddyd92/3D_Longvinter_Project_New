#include "stdafx.h"
#include "..\Public\Tool_Granade.h"

#include "Player.h"

#include "GameInstance.h"

CTool_Granade::CTool_Granade(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CTool{ pDevice, pContext }
{
}

CTool_Granade::CTool_Granade(const CTool_Granade& Prototype)
	: CTool{ Prototype }
{
}

HRESULT CTool_Granade::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTool_Granade::Initialize(void* pArg)
{
...
void CTool_Granade::Priority_Update(_float fTimeDelta)
{

	int a = 10;
}

void CTool_Granade::Update(_float fTimeDelta)
{
...
void CTool_Granade::Late_Update(_float fTimeDelta)
{
...
HRESULT CTool_Granade::Ready_Components()
{
	/* FOR.Com_Shader */
	...
	/* FOR.Com_Model */
	if (FAILED(__super::Add_Component(_int(LEVELID::LEVEL_STATIC), TEXT("Prototype_Component_Model_Throw_Granade_Pinned"),
...
CTool_Granade* CTool_Granade::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CTool_Granade* pInstance = new CTool_Granade(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CTool_Granade"));
...
CGameObject* CTool_Granade::Clone(void* pArg)
{
	CTool_Granade* pInstance = new CTool_Granade(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CTool_Granade"));
		Safe_Release(pInstance);
	}
